...
public class DAOCliente extends Cliente {
    ...
    public String consultaCliente() {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                String estadoCli = rs.getString(7);
    ...
    public void listarCliente(String valor, String filtro, JTable tabla, String estado) {
        String[] columnas = {"Nombre", "Apellido", "Cedula", "Telefono", "Correo", "Sexo",
            "Plan", "Fecha Afiliación", "Estado Plan"};
        ...
        try {
            ...
            while (rs.next()) {
                ...
                registros[6] = rs.getString("planC");
                registros[7] = rs.getString("fechaAfi");
                registros[8] = rs.getString("estadoPlan");
    ...
    public void generarPdf(String dato, String filtro, String estado) {
        ...
        try {
            ...
            tabla.addCell("Plan");
            tabla.addCell("Fecha Afiliación");
            tabla.addCell("Estado Plan");
            tabla.addCell("Estado Afiliación");

            if (filtro.equals("Nombre")) {
                //NOTA: Escribir la onstrucción sql segida para que no haya error,
                //o si no la instrucción llega incompleta a postgreSQL
                SQL = "SELECT * FROM Cliente INNER JOIN Plan ON plan.cedula = Cliente.cedulaCli WHERE nombreCli LIKE '%"+dato+"%' AND estadoClie = '" + estado + "';";
            } else if (filtro.equals("Sexo")) {
                SQL = "SELECT * FROM Cliente INNER JOIN Plan ON plan.cedula = Cliente.cedulaCli WHERE sexoCli LIKE '%"+dato+"%' AND estadoClie = '" + estado + "';";
            } else {
                SQL = "SELECT * FROM Cliente INNER JOIN Plan ON plan.cedula = Cliente.cedulaCli WHERE estadoClie = '" + estado + "';";
            }
